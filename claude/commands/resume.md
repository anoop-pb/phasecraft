...
6. Do NOT consume the Project Manager inbox in this session — `/resume` is tracker-based status/routing only. PM-addressed messages are consumed on `/plan`. (You may glance at counts for context, but never delete a message here.)
7. Report status and recommend next command per your agent definition
